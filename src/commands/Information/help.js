  ...
  execute: async (message, args, client, prefix) => {
    ...
    const m = await message.reply({
      ...
      components: [
        ...
        new ActionRowBuilder().addComponents(but6),
      ],
    });
      ...
      await m
        .edit({
          components: [
            ...
            new ActionRowBuilder().addComponents(but6.setDisabled(true)),
          ],
        })
        .catch(() => {});
    });
    collector.on("collect", async (b) => {
      ...
      let row2 = new ActionRowBuilder().addComponents(but6);
      ...
      if (b.customId == "registration") {
        _commands = client.commands
          .filter((x) => x.category && x.category === "Registration")
          .map((x) => `\`${x.name}\``);
        editEmbed
          .setColor(client.embedColor)
          .setDescription(_commands.join(", "))
          .setTitle("Tournament Registration Commands")
          .setFooter({
            text: `Total ${_commands.length} Registration commands.`,
          });
        return await m.edit({
          embeds: [editEmbed],
          components: [row, row2],
