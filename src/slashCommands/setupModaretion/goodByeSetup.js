...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (options.getString("goodbay-text-color")) {
      if (!checker.test(options.get("goodbay-text-color").value))
        return interaction.editReply({
          embeds: [successEmbedLL],
        });
      data.welcome_color = options.getString("goodbay-text-color");
    }
    ...
    storeOf.goodbay = data;
    interaction.reply({
      embeds: [
        new EmbedBuilder()
          .setTitle("Goodbayer Deployed")
          .setDescription(
            `Goodbayer Deployed In ${options.getChannel("channel")}`
          )
          .setColor(0xff6600)
          .setFooter({
            text: client.user.username,
   ...
   storeOf.save();
   {
    ...
    let wlcData = replaceR(storeOf.goodbay, interaction.member);

    const { wlcBenner } = require("../../tamplates/welcomeTemp1");
