...
async function check(client, guild, textchannel, voicechannel) {
  let isGuild;
  let isChannel;
  const resolveGuild = await client.guilds
    .fetch(guild)
    .catch(() => (isGuild = false));

  if (isGuild === false) return false;

  await resolveGuild.channels
    .fetch(textchannel)
    .catch(() => (isChannel = false));
  await resolveGuild.channels
    .fetch(voicechannel)
    .catch(() => (isChannel = false));
 ...
 * @param {TheTurBOT} client Extended Client
 * @returns {boolean}
 */
...
function neb(embed, player, client) {
  ...
  return embed
    .setDescription(
      `[${player.queue.current.title}](${player.queue.current.uri}) by ${
